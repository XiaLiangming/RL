...
def train(device: torch.device,
          ...
          grad_clip: float = 1.0,
          history_path: str = "./history/dqn_vanilla.csv",
          best_model_save_path: str = "./models/dqn_vanilla_best.pth"):
    ...
    model = Model(input_shape=(frame_stacking, *observation_space),
                  action_space=action_space).to(device)
    optimizer = optim.Adam(model.parameters(), lr=0.0000625, eps=1.5e-4)
        ...
        while not done:
            ...
            if step % eval_steps == 0:
                ...
                for _ in range(eval_episode):
                    total_eval_reward += evaluate_model(model, Env, frame_stacking, eval_epsilon, device)
...
if __name__ == "__main__":
    frame_stacking = 4
    eval_epsilon = 0.05
    history_path = "./history/dqn_vanilla.csv"
    best_model_save_path = "./models/dqn_vanilla_best.pth"
    gif_save_path = "./gifs/dqn_vanilla.gif"
    plot_save_path = "./plots/dqn_vanilla.png"
