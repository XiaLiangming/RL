...
def train(device: torch.device,
          ...
          grad_clip: float = 1.0,
          history_path: str = "./history/dqn_per_clip.csv",
          best_model_save_path: str = "./models/dqn_per_clip_best.pth"):

    # Create history directory and model directory if it doesn't exist
    os.makedirs(os.path.dirname(history_path), exist_ok=True)
    os.makedirs(os.path.dirname(best_model_save_path), exist_ok=True)
    # Initialize environments
    env = Env(clip_rewards=True)
        ...
        while not done:
            ...
            if step > warmup_steps:
                ...
                if step % train_steps == 0:
                    states, actions, rewards, next_states, dones, weights = buffer.sample(batch_size)
                    ...
                    loss = loss_function(q_values, target_q_values, reduction='none')
            ...
            if step % eval_steps == 0:
                ...
                with open(history_path, "a") as f:
                    f.write(f"{episode:<10}\t{avg_train_reward:<15.4f}\t{avg_loss:<15.6f}\t{step:<10}\t{epsilon:<10.4f}\t{avg_eval_reward:<15.2f}\n")
                print(f"Episode: {episode}, AvgTrainReward: {avg_train_reward:.4f}, AvgLoss: {avg_loss:.6f}, Step: {step}, Epsilon: {epsilon:.4f}, AvgEvalReward: {avg_eval_reward:.2f}")
                total_train_reward = 0
                total_loss = 0
    env.close()
...
if __name__ == "__main__":
    ...
    frame_stacking = 4
    eval_epsilon = 0.05
    history_path = "./history/dqn_per_clip.csv"
    best_model_save_path = "./models/dqn_per_clip_best.pth"
    gif_save_path = "./gifs/dqn_per_clip.gif"
    plot_save_path = "./plots/dqn_per_clip.png"
